...
def check(config):
    ...
    if smtp_states == "1":
        ...
        smtp_process = Process(target=smtp.SMTP, args=(host, smtp_port, log_filepath, "smtp", logs))
        smtp_process.start()
    ...
    if ftp_states == "1":
        ...
        ftp_process = Process(target=ftp.FTP, args=(host, ftp_port, log_filepath, "ftp", logs))
        ftp_process.start()
    ...
    if telnet_states == "1":
        ...
        telnet_process = Process(target=telnet.Telnet, args=(host, telnet_port, log_filepath, "telnet", logs))
    ...
    if rdp_states == "1":
        ...
        rdp_process = Process(target=rdp.RDP, args=(host, rdp_port, log_filepath, "rdp", logs))
        rdp_process.start()
    ...
    if redis_states == "1":
        ...
        redis_process = Process(target=redis.Redis, args=(host, redis_port, log_filepath, "redis", logs))
        redis_process.start()
    ...
    if pop3_states == "1":
        ...
        pop3_process = Process(target=pop3.POP3, args=(host, pop3_port, log_filepath, "pop3", logs))
        pop3_process.start()
    ...
    if tftp_states == "1":
        ...
        tftp_process = Process(target=tftp.TFTP, args=(host, tftp_port, log_filepath, "tftp", logs))
        tftp_process.start()
